...
# - Quality control (QC) is performed if the corresponding variable (QC.MODE, see
#   below) set to TRUE. QC is only applied to the lumen area profile, and all other 
#   profile plotting and calculation of annual value (summaries) is skipped. QC 
#   includes flagging intra-ring bands with low replication, (large) outliers, 
#   level offset between neighboring images, data gaps and potential duplicate
#   rings due to wrong cross-dating. See below for explanation of the different 
#   flags.
# 
# 
#%#%# CUSTOMIZABLE GENERAL OPTIONS:
# 
...
# RESO - resolution(s) / band width(s) (?m) of the intra-ring profiles
# SMOOTHER - smoothing spline for the intra-ring profiles
# QC.MODE - quality control mode that will only plot the intra-annual profile of ring width together with several features useful for quality control; also the extraction and plotting of annual statistics is skipped in this mode
...
#
#
#%#%# CUSTOMIZABLE QUALITY CONTROL (QC) OPTIONS APPLIED ON LUMEN AREA (LA) PROFILES:
# 
# (A) LOW REPLICATION
# NCELL.MIN	- minimum number of cells per band; if below, the band will be flagged (see next line for style options)
# NCELL.DIST - intra-ring band distance [?m] from inner ring, for which flagged bands receive yellow color (for all others: magenta); this is useful since at high resolution the first band(s) can by definition no contain many cells, and flagging them is not very informative
# 
# (B) LEVEL OFFSET IN LA BETWEEN ADJACENT IMAGES
# LA.REF - lumen area quantile per image (segments) for horizontal reference line
# LA.THR - maximum allowed deviation in lumen area quantile between neighboring image (segments); if deviating more, the reference quantile line will be highlighted; for the very first and very last band, the double deviation is taken
# 
# (C) DUPLICATE RINGS
# DUP.RW - maximum deviation in ring width of neighboring rings from different images to consider them as potential duplicates due to wrong visual cross-dating   
# DUP.KS - minimum p-value of Kolmogorov-Smirnoff test between lumen area profiles of neighboring rings from different images to highlight them as potential duplicates (cross-dating mistake!) 
# 
# (D) LARGE OUTLIERS
# RM.WIDTH - width [?m] of running mean smoothing filter applied on LA; if not an uneven multiple of the intra-ring band width (RESO), rounding is applied; the resulting smoothed LA profiles serves as a reference to identify (large) outliers 
# OUT.THR.REL - threshold ratio between target LA band and smoothed LA band above which to flag it as a potential outlier
# OUT.THR.ABS - if the target LA band surpasses the smoothed LA band by more than OUT.THR.ABS x LA90 (90th percentile of LA), the target band is flagged it as a potential outlier
# OUT.QU - only flag potential outliers if larger than the specified quantile (on ring level)
#
#
#%#%# FLAGS & FEATURES IN QUALITY CONTROL (QC) MODE:
# GREY-WHITE BACKGROUND  - blocks of data taken from the same image (see labels in top right corner for image ID) 
# YELLOW CIRCLES         - low replication in first few earlywood bands
# MAGENTA CIRCLES        - low replication in any other band than the first few earlywood bands
# CYAN CIRCLES           - gap in profile data (intra-ring band with no cells, excluded rings, etc.)
# GREEN TRIANGLE         - potential outlier (intra-ring band with much larger lumen area value than in the neighboring bands)
# DASHED HORIZONTAL LINE - 90th percentile of lumen area for each image / block of data
# YELLOW HORIZONTAL LINE - image / block with considerably lower mean level of lumen area compared to neighboring images / blocks
# YELLOW PROFILES        - very similar lumen area profiles and ring width of two neighboring rings from different images, which could indicate a cross-dating mistake
#
#
#%#%# INCLUDED PARAMETERS: 
# 
# MRW - ring width (integrating tangential fluctuations) [?m]
# LA - lumen area [?m2]
# TCA - total cell area (lumen area + cell wall area) [?m2]
# DRAD - radial lumen diameter [?m]
# DTAN - tangential lumen diameter [?m]
# CWA - cell wall area (unit: cell)  [?m2]
# CWAACC - accumulated cell wall area (unit: sum of each intra-ring band) [?m2]
# CWTALL - average cell wall thickness (mean of tangential and radial) [?m]
# CWTALL.ADJ - adjusted average cell wall thickness (earlywood: tangential, latewood: mean of tangential and radial) [?m]
# CWTTAN - tangential cell wall thickness [?m]
# CWTRAD - radial cell wall thickness [?m]
...
# DH - mean hydraulic diameter ((S(D^4)/n)^0.25, where S is the sum, D is the hydraulic cell diameter, n the number of cells) [?m]
...
homedir <- '/media/tim/dataDisk/PlantGrowth/data/microcores/woodAnatomy/Exp2018/ROXAS/'
setwd (homedir)

### 3. Let user customize some options ####
### 3.1 General options ####
YR.START <- 0   #minimum potential start year of profiles
YR.END <- 3000   #maximum potentitial ending year of profiles
...
# RESO <- c(10,20)   #spatial resolution of profiles (micrometers), i.e. width of intra-ring bands

SMOOTHER <- 1   #uneven numbers only! Defines the width of the rolling mean filter with respect to RESO; 1 means no smoothing

QC.MODE <- FALSE   #when this option is activated, only the intra-annual profile of ring width together with several features useful for quality control are plotted; also the extraction and plotting of annual statistics is skipped in this mode
...
### 3.2 Quality control (QC) options applied to intra-ring lumen area (LA) profiles ####
NCELL.MIN	<- 5   #minimum number of cells per band; if below, the band will be flagged (see next line for style options)
NCELL.DIST <- 30   #intra-ring band distance [?m] from inner ring, for which flagged bands receive yellow color (for all others: magenta); this is useful since at high resolution the first band(s) can by definition no contain many cells, and flagging them is not very informative

LA.REF	<- 0.9   #lumen area quantile per image (segments) for horizontal reference line
LA.THR <- 0.9   #maximum allowed deviation in lumen area quantile between neighboring image (segments); if deviating more, the reference quantile line will be highlighted; for the very first and very last band, the double deviation is taken

DUP.RW	<- 0.04   #maximum deviation in ring width of neighboring rings from different images to consider them as potential duplicates due to wrong visual cross-dating   
DUP.KS	<- 0.98   #minimum p-value of Kolmogorov-Smirnoff test between lumen area profiles of neighboring rings from different images to highlight them as potential duplicates (cross-dating mistake!) 

RM.WIDTH	<- 70   #width [?m] of running mean smoothing filter applied on LA; if not an uneven multiple of the intra-ring band width (RESO), rounding is applied; the resulting smoothed LA profiles serves as a reference to identify (large) outliers 
OUT.THR.REL	<- 2.0   #threshold ratio between target LA band and smoothed LA band above which to flag it as a potential outlier
OUT.THR.ABS <- 0.67   #if the target LA band surpasses the smoothed LA band by more than OUT.THR.ABS x LA90 (90th percentile of LA), the target band is flagged it as a potential outlier
OUT.QU	<- 0.75   #only flag potential outliers if larger than the specified quantile (on ring level)
...
exclude <- data.frame ("WOODID"= NA, "YEAR" = NA)
...
for (i in c(1:length(cell.files))) {
  ### 4.1 Load data ####    
  ...
  if (QC.MODE == FALSE) {   #only execute if not in qc mode
  ...
  for (s in c(1:length(STATS))) {  
    ...
    for (r in c(1:length(RESO))) {
      ...
      suppressWarnings(dt <- dt[, list(RADDISTR.ST.LAST=max(RADDISTR.ST, na.rm=TRUE), RADDISTR.BAND.MAX=max(RADDISTR.BAND, na.rm=TRUE)), by=key(dt)])
      ...
      if (QC.MODE==FALSE & (sum(dt$RTSR,na.rm=T) > 0))   #only execute if not in qc mode and if there are non-NA Mork's index values 
      {
        ...
        if (s==1 & r==1)  
        {
          ...
          ### 4.6.1.4.1 Create data frame with median Mork's index (RTSR) in 10-?m steps   
          dt <- NULL
          dt <- data.table(cells[,c("YEAR", "MRW", "RRADDISTR", "RADDISTR.ST", "RTSR")])
          dt$YEAR <- as.factor(dt$YEAR)
          dt$RADDISTR.BAND <- 10 * round((dt$RADDISTR.ST- 10 / 2.01) / 10 , 0)   #add column where each cell is assigned to 10-?m band; insert row if one band is missing
      ...
      }else {  #if no EW parameters were calculated, add corresponding NA columns
        df0$EWW <- NA
        df0$LWW <- NA
        df0$EWLW.BAND <- NA
        df0$EWLW.RPOS <- NA
        df0$EWLW.ID <- NA
      }   #end of if (QC.MODE==FALSE)   
      ...
      df1 <- df1[order(as.numeric(df1$YEAR)),]
      ...
      df0$YEAR <- as.character(df0$YEAR) 
      ...
      df2 <- df2[order(as.numeric(df2$YEAR), df2$RADDISTR.BAND),]   #bring data frame into original order: year-band
      ...
      suppressWarnings(iap$YR.RRADDISTR.BAND <- as.numeric(paste(as.character(iap$YEAR), as.character(iap$YR.RRADDISTR.BAND), sep=".")))   #merge year and rel. position information
      ...
      if (QC.MODE==TRUE)   #only execute if in qc mode       
      {  
        ...
        imgid <- iap2[,c("YEAR","IMAGE","RADDIST.CONT","X.LABEL")]   #subset intra-annual dataframe
        imgid <- imgid[!is.na(imgid$X.LABEL),]   #extract rows at the beginning of each ring
        imgid$BLOCK <- 1   #initialize grouping column
        
        for (n in c(2:nrow(imgid)))   #give same index to rows getting data from the same image; if data source flips between two images where they overlap, treat each segment as independent group
        {
          if (imgid$IMAGE[n]!=imgid$IMAGE[n-1]) 
          {
            imgid$BLOCK[n] <- imgid$BLOCK[n-1] + 1
          }else
          {
            imgid$BLOCK[n] <- imgid$BLOCK[n-1]
          }  
        }  
        
        imgid$BLOCK <- as.factor(imgid$BLOCK)   #convert grouping index to factor
        imgid <- imgid[!duplicated(imgid[c("BLOCK")]),]   #extract the first row of each group
        
        if (nrow(imgid)>1)   #if there is more than one group, define corner coordinates of grouping background rectangles
        {  
          topleft <- imgid$RADDIST.CONT[seq(2,nrow(imgid),2)]   #coordinate of every other group at top left corner
          bottomright <- imgid$RADDIST.CONT[seq(3,nrow(imgid)+1,2)]   #coordinate of every other group at bottom right corner
        ...
        {  
          minno <- iap2[which(iap2$N.BAND < NCELL.MIN & !is.na(iap2$LA)),c("RADDIST.CONT","RADDISTR.BAND","N.BAND","LA")]
        }
        
        ### 4.6.1.25.2.3 Calculate nth (e.g., 90th) percentile of lumen area per image or contiguous image segment, respectively ####
        ### Calculation of percentile lumen area
        imla90 <- NULL
        dt <- NULL
        dt <- data.table(iap2)
        ...
        dt <- dt[, list(IMAGE=unique(IMAGE)   #aggregate per image segment and calculate lumen area quantile
                 ...
                 by=key(dt)]
        imla90 <- as.data.frame(setorder(dt, -BLOCK2))        
        
        ### Identify image-segment level lumen area quantiles deviating strongly from neighboring segments
        imla90$FLAG <- "no"   #set default: not deviating strongly
        if (nrow(imla90) >=2)   #only apply if minimum 2 segments
        {  
          if (!is.na(imla90$LA90[1]) & !is.na(imla90$LA90[2]) & imla90$LA90[1]/imla90$LA90[2] < (1-2*(1-LA.THR)))  #at beginning: if 1st segments is much smaller than 2nd segment; verify also that lumen area quantiles are not NA 
          {
            imla90$FLAG[1] <- "yes"
          }  
          if (!is.na(imla90$LA90[nrow(imla90)]) & !is.na(imla90$LA90[nrow(imla90)-1]) & imla90$LA90[nrow(imla90)]/imla90$LA90[nrow(imla90)-1] < (1-2*(1-LA.THR)))  #at end: if last segments is much smaller than 2nd last segment; verify also that lumen area quantiles are not NA
          {
            imla90$FLAG[nrow(imla90)] <- "yes"
          }
        }
        if (nrow(imla90) >= 3)   #for the non-marginal segments
        {  
          for (n in c(2:(nrow(imla90)-1)))
          { 
            if(!is.na(imla90$LA90[n]) & !is.na(imla90$LA90[n-1]) & !is.na(imla90$LA90[n+1]))   #verify that the considered lumen area quantiles are not NA
            {  
              if ((2 * imla90$LA90[n] / (imla90$LA90[n-1] + imla90$LA90[n+1])) < LA.THR)   #highlight segment if considerable smaller or larger lumen area quantile than for the average of its neighboring segments 
              {  
                imla90$FLAG[n] <- "yes"
              }
            }
        ...
        {
          gap <- iap2[which(is.na(iap2$LA)), c("RADDIST.CONT","LA")]   #extract rows with NA for lumen area
          gap$ID <- as.numeric(row.names(gap))
          if (nrow(gap) > 1)   #check whether there is more than 1 NA
          {  
            for (n in 1:c(nrow(gap)-1))
            {
              if (gap$ID[n]+1 == gap$ID[n+1])   #assign -999 if NA-row is following another directly adjacent NA-row
              {
                gap$ID[n] <- -999
              }
            }  
          } 
          gap <- gap[which(gap$ID>0), "ID"] + 1   #extract NA-rows that are not flagged -999
          gap <- iap2$RADDIST.CONT[gap]   #extract radial distance of first bands after a gap
          gap <- gap[!is.na(gap)]
        }
        ...
        for (n in c(2:length(unique(iap2$YEAR))))   #loop through all years
        {
          ...
          {
            if (unique(iap2$IMAGE[iap2$YEAR==unique(iap2$YEAR)[n]]) != unique(iap2$IMAGE[iap2$YEAR==unique(iap2$YEAR)[n-1]]))   #only evaluate if neighboring rings are from different images
            {  
              ...
              {  
                if (suppressWarnings(ks.test(iap2$LA[iap2$YEAR==unique(iap2$YEAR)[n]], iap2$LA[iap2$YEAR==unique(iap2$YEAR)[n-1]])$p.value > DUP.KS))   #perform Kolmogorov-Smirnoff test and compare p-value against threshold; suppressWarnings to avoid warnings if p-value is only approx. because of ties
                {  
                  k <- k+1
                  duplr[k] <- unique(iap2$YEAR)[n] - 1   #add calendar year to list
                  k <- k+1
                  duplr[k] <- unique(iap2$YEAR)[n]   #add calendar year to list
                }
              }
            }
          }
        ...
        ### Calculate smoothed profile and extract percentiles per ring
        dt <- NULL
        dt <- iap2[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END, c("YEAR","RADDISTR.BAND","RADDIST.CONT","LA")]
        RM.KERNEL <- 2 * round(RM.WIDTH/(2*RESO[r])-0.5, 0) + 1   #find size of smoothing kernel; must be uneven number!
        if (RM.KERNEL>1)   #only perform smoothing if smoothing kernel includes any neighboring bands
        {  
          margin <- c(mean(dt$LA[1:((RM.KERNEL-1)/2)], na.rm=TRUE), NA ,mean(dt$LA[c((ln<-length(dt$LA)):(ln-((RM.KERNEL-1)/2)))], na.rm=TRUE))
          dt$LA.SMOOTHED <- rollapply(dt$LA, RM.KERNEL, fill=margin, mean, na.rm=TRUE)
        }
        dt$LA.SMOOTHED <- na.locf(dt$LA.SMOOTHED)    #replace NA by first preceeding non-NA value per ring
        dt$LA.SMOOTHED.ADJ <- dt$LA.SMOOTHED   #prepared adjusted smoothed
        dt$LA.SMOOTHED.ADJ[dt$RADDISTR.BAND <= (RESO[r] * ((RM.KERNEL-1) / 2 - 1))] <- NA   #set values of first smoothed bands to NA because they are still influenced by previous ring
        dt$LA.SMOOTHED.ADJ[nrow(dt)] <- ifelse(is.na(dt$LA.SMOOTHED.ADJ[nrow(dt)]), dt$LA.SMOOTHED[nrow(dt)], dt$LA.SMOOTHED.ADJ[nrow(dt)])   #if the last values were replaced by NA, assign last value again LA.SMOOTHED, otherwise the next line will through an error
        dt$LA.SMOOTHED.ADJ <- na.locf(dt$LA.SMOOTHED.ADJ, fromLast=TRUE)    #replace NA by first following non-NA value per ring
        dt <- data.table(ddply(dt, "YEAR", mutate, LA90=quantile(LA, 0.90, na.rm=TRUE),
                               LA.OUT.QU=quantile(LA, OUT.QU, na.rm=TRUE)))   #find percentiles per ring
        smoo <- data.frame(dt)
        
        ### Extract (large) outliers
        outl <- which((smoo$LA / smoo$LA.SMOOTHED.ADJ > OUT.THR.REL)   #ratio between target band and smoothed band 
                      | (smoo$LA - smoo$LA.SMOOTHED.ADJ > smoo$LA90 * OUT.THR.ABS)   #absolute difference between band and smoothed band 
                      & smoo$LA > smoo$LA.OUT.QU)   #only consider outliers that correspond to the OUT.QU percentile (e.g., 75th) of the respective ring
      }   #end of if (QC.MODE==TRUE) 
      
      
      ### 4.6.1.26 Plot and save intra-annual profile of LA (lumen area); add additional QC features if opted for ####
      if (QC.MODE==TRUE)   #only execute if in qc mode
      {  
        png(file=paste("QC_", woodid, "_Intraprofile_LA_", STATS[s], "_", RESO[r], "mu_", SMOOTHER, "rm", ".png", sep=""), width=im.width, height=im.height)
      }else
      {
      ...
      plot(y=iap2$LA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END], 
           ...
           xaxt="n", yaxt="n", xlab="", ylab="Lumen Area (LA) (?m2)", type="l", lwd=0, col=LINE.COL(length(RESO))[r], 
      ...
      {  
        ...
        text(x=imgid$RADDIST.CONT,y=max(iap2$LA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T),imgid$IMAGE,adj=c(0,0),cex=3)
        
        ### Highlight image-segment level lumen area quantiles if deviating strongly from neighboring segments
        for (n in c(which(imla90$FLAG=="yes")))
        { 
          lines(x=c(imla90$X.START[n], imla90$X.END[n]), y=c(imla90$LA90[n], imla90$LA90[n]), lty=1, lwd=60, col="yellow")
        }
        
        ### Add horizontal lines for all image-segments with their average lumen area percentile
        for (n in c(1:nrow(imla90)))
        {  
          lines(x=c(imla90$X.START[n], imla90$X.END[n]), y=c(imla90$LA90[n], imla90$LA90[n]), lty=2, lwd=6)
        }
        
        ### Add yellow circle for bands based on very few individual cells, positioned at y = corresponding lumen area 
        if (length(minno) > 0)
        {  
          points(minno$RADDIST.CONT[minno$RADDISTR.BAND>NCELL.DIST], minno$LA[minno$RADDISTR.BAND>NCELL.DIST], pch=21, col="black", bg="magenta", lwd=5, cex=40)
          points(minno$RADDIST.CONT[minno$RADDISTR.BAND<=NCELL.DIST], minno$LA[minno$RADDISTR.BAND<=NCELL.DIST], pch=21, col="black", bg="yellow", lwd=5, cex=40)
        }
        
        ### Add cyan circle at beginning of previously interrupted (gap) profile at y = 0
        if (length(gap) > 0)
        {  
          points(gap, rep(lim[3]+10,length(gap)), pch=21, col="black", bg="cyan", lwd=5, cex=40)
        }
        
        ### Highlight neighboring rings that are potential duplicated due to a visual cross-dating mistake
        if (length(duplr) > 0)
        {  
          for (n in c(1:length(duplr)))
          {  
            lines(y=iap2$LA[iap2$YEAR==duplr[n]], x=iap2$RADDIST.CONT[iap2$YEAR==duplr[n]], lwd=60, col="yellow")
          }
        }
        
        ### Add green triangles to mark potential (large) outliers
        lines(y=smoo$LA.SMOOTHED, x=smoo$RADDIST.CONT, col="black", lwd=3)   #add smoothed reference line
        points(x=iap2$RADDIST.CONT[outl], y=iap2$LA[outl], pch=25, col="black", bg="green", lwd=5, cex=30)   #plot outliers
      }   #end of if (QC.MODE==TRUE)
      
      lines(y=iap2$LA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END], 
            x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END], 
            xaxt="n", yaxt="n", xlab="", ylab="Lumen Area (LA) (?m2)", type="l", lwd=4, col=LINE.COL(length(RESO))[r], 
      ...
      if (QC.MODE==TRUE)   #if in qc mode, plot smoothed curve over the original profile
      {        
        lines(y=smoo$LA.SMOOTHED, x=smoo$RADDIST.CONT, col="black", lwd=3)   #add smoothed reference line on top
      }
      dev.off()
      
      
      #### *** ONLY PERFORM THE PLOTTING OF ALL OTHER PROFILES & THE CALCULATIONS ON ANNUAL LEVEL IF NOT IN QC MODE! *** ####
      if (QC.MODE==FALSE)   #only execute if in qc mode   
      {  
        ...
        plot(y=iap2$DRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
             x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
             xaxt="n", yaxt="n", xlab="", ylab="Radial lumen diameter (DRAD) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
             # cex.axis=3, cex.lab=3, ylim=c(25,1500))
        ...
        lines(y=iap2$DRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
              x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
              xaxt="n", yaxt="n", xlab="", ylab="Radial lumen diameter (DRAD) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
              cex.axis=3, cex.lab=3)
        par(xaxt="s")
        axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
             , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        plot(y=iap2$DTAN[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
             x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
             xaxt="n", yaxt="n", xlab="", ylab="Tangential lumen diameter (DTAN) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
             # cex.axis=3, cex.lab=3, ylim=c(25,1500))
        ...
        lines(y=iap2$DTAN[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
              x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
              xaxt="n", yaxt="n", xlab="", ylab="Tangential lumen diameter (DTAN) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
              cex.axis=3, cex.lab=3)
        par(xaxt="s")
        axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
             , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$TCA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA TCA data
        {  
          ...
          plot(y=iap2$TCA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Cell Area (TCA) (?m2)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
          ...
          lines(y=iap2$TCA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Cell Area (TCA) (?m2)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWA data
        {  
          ...
          plot(y=iap2$CWA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Cell Wall Area (CWA) (?m2)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
          ...
          lines(y=iap2$CWA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Cell Wall Area (CWA) (?m2)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWAACC[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWAACC data
        {  
          ...
          plot(y=iap2$CWAACC[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Accumulated Cell Wall Area (CWAACC) (?m2)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
               # cex.axis=3, cex.lab=3, ylim=c(0,2000))
          ...
          lines(y=iap2$CWAACC[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Accumulated Cell Wall Area (CWAACC) (?m2)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWTALL[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWTALL data
        {  
          ...
          plot(y=iap2$CWTALL[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Overall Cell Wall thickness (CWTALL) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
          ...
          lines(y=iap2$CWTALL[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Overall Cell Wall thickness (CWTALL) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWTALL.ADJ[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWTALL.ADJ data
        {  
          ...
          plot(y=iap2$CWTALL.ADJ[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Overall adjusted Cell Wall thickness (CWTALL.ADJ) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
               # cex.axis=3, cex.lab=3, ylim=c(1.5,10.0))
          ...
          lines(y=iap2$CWTALL.ADJ.ADJ[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Overall adjusted Cell Wall thickness (CWTALL.ADJ) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWTRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWTRAD data
        {  
          ...
          plot(y=iap2$CWTRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Radial Cell Wall thickness (CWTRAD) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
               # cex.axis=3, cex.lab=3, ylim=c(1.5,10.0))
          ...
          lines(y=iap2$CWTRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Radial Cell Wall thickness (CWTRAD) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$CWTTAN[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CWTTAN data
        {  
          ...
          plot(y=iap2$CWTTAN[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Tangential Cell Wall thickness (CWTTAN) (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
               # cex.axis=3, cex.lab=3, ylim=c(1.5,9.0))
               cex.axis=3, cex.lab=3, ylim=c(1.5,max(iap2$CWTRAD[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T)))
          lines(y=iap2$CWTTAN[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Tangential Cell Wall thickness (CWTTAN) (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        if (sum(iap2$RTSR[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA RTSR data
        {  
        ...
        if (sum(iap2$CTSR[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA CTSR data
        {  
        ...
        if (sum(iap2$DCWT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA DCWT data
        {  
        ...
        if (sum(iap2$DCWT2[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA DCWT2 data
        {  
        ...
        if (sum(iap2$DCWA[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA DCWA data
        {  
        ...
        if (sum(iap2$TB2[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA TB2 data
        {  
        ...
        if (sum(iap2$KH[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA KH data
        {  
        ...
        if (sum(iap2$DH[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],na.rm=T) > 0)   #only plot if there is any non-NA DH data
        {  
          ...
          plot(y=iap2$DH[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
               xaxt="n", yaxt="n", xlab="", ylab="Mean hydraulic diameter (?m)", type="l", lwd=0, col=LINE.COL(length(RESO))[r],
               # cex.axis=3, cex.lab=3, ylim=c(0.1,0.95))
          ...
          lines(y=iap2$DH[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                x=iap2$RADDIST.CONT[iap2$YEAR>=YR.START & iap2$YEAR<=YR.END],
                xaxt="n", yaxt="n", xlab="", ylab="Mean hydraulic diameter (?m)", type="l", lwd=4, col=LINE.COL(length(RESO))[r],
                cex.axis=3, cex.lab=3)
          par(xaxt="s")
          axis(1, at=iap2$RADDIST.CONT[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END]
               , tck=0.015, lwd.ticks=2, cex.axis=2, labels=iap2$X.LABEL[!is.na(iap2$X.LABEL) & iap2$YEAR>=YR.START & iap2$YEAR<=YR.END])
        ...
        plot(y=mrw$MRW[mrw$YEAR>=YR.START & mrw$YEAR<=YR.END],
             ...
             xaxt="n",  yaxt="n", xlab="",ylab="Ring width (MRW) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(eww$EWW[eww$YEAR>=YR.START & eww$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA EWW data
        {  
          ...
          plot(y=eww$EWW[eww$YEAR>=YR.START & eww$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Earlywood width (EWW) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #ylim=c(150,1500))
          par(xaxt="s")
        ...
        if (sum(lww$LWW[lww$YEAR>=YR.START & lww$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA LWW data
        {  
          ...
          plot(y=lww$LWW[lww$YEAR>=YR.START & lww$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Latewood width (LWW) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #ylim=c(150,1500))
          par(xaxt="s")
        ...
        plot(y=mxla$MXLA[mxla$YEAR>=YR.START & mxla$YEAR<=YR.END],
             ...
             xaxt="n",  yaxt="n", xlab="",ylab="Max. Lumen area (MXLA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
             cex.axis=1.3, cex.lab=2) #, ylim=c(150,1500))
        par(xaxt="s")
        ...
        plot(y=mnla$MNLA[mnla$YEAR>=YR.START & mnla$YEAR<=YR.END],
             ...
             xaxt="n",  yaxt="n", xlab="",ylab="Min. Lumen area (MNLA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mxtca$MXTCA[mxtca$YEAR>=YR.START & mxtca$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXTCA data
        {  
          ...
          plot(y=mxtca$MXTCA[mxtca$YEAR>=YR.START & mxtca$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Max. Cell area (MXTCA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mntca$MNTCA[mntca$YEAR>=YR.START & mntca$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNTCA data
        {  
          ...
          plot(y=mntca$MNTCA[mntca$YEAR>=YR.START & mntca$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Min. Cell area (MNTCA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mxcwa$MXCWA[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXCWA data
        {  
          ...
          plot(y=mxcwa$MXCWA[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Max. Acc. Cell wall area (MXCWAACC) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mncwa$MNCWA[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNCWA data
        {  
          ...
          plot(y=mncwa$MNCWA[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Min. Acc. Cell wall area (MNCWAACC) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        {  
          ...
          plot(y=mxcwa$MXCWA[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END],
               x=mxcwa$YEAR[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END],
               xaxt="n",  yaxt="n", xlab="",ylab="Max. Cell wall area (MXCWA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #, ylim=c(100,4000))
          par(xaxt="s")
          axis(1, at=mxcwa$YEAR[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END], tck=0.02, lwd.ticks=2, cex.axis=1.3,
               labels=mxcwa$YEAR[mxcwa$YEAR>=YR.START & mxcwa$YEAR<=YR.END])
        ...
        {  
          ...
          plot(y=mncwa$MNCWA[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END],
               x=mncwa$YEAR[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END],
               xaxt="n",  yaxt="n", xlab="",ylab="Min. Cell wall area (MNCWA) (?m2)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #, ylim=c(20,500))
          par(xaxt="s")
          axis(1, at=mncwa$YEAR[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END], tck=0.02, lwd.ticks=2, cex.axis=1.3,
               labels=mncwa$YEAR[mncwa$YEAR>=YR.START & mncwa$YEAR<=YR.END])
        ...
        if (sum(mxcwttan$MXCWTTAN[mxcwttan$YEAR>=YR.START & mxcwttan$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXCWTTAN data
        {  
          ...
          plot(y=mxcwttan$MXCWTTAN[mxcwttan$YEAR>=YR.START & mxcwttan$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Max. tangential cell wall thickness (MXCWTTAN) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mncwttan$MNCWTTAN[mncwttan$YEAR>=YR.START & mncwttan$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNCWTTAN data
        {  
          ...
          plot(y=mncwttan$MNCWTTAN[mncwttan$YEAR>=YR.START & mncwttan$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Min. tangential cell wall thickness (MNCWTTAN) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
        ...
        if (sum(mxcwtrad$MXCWTRAD[mxcwtrad$YEAR>=YR.START & mxcwtrad$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXCWTRAD data
        {  
          ...
          plot(y=mxcwtrad$MXCWTRAD[mxcwtrad$YEAR>=YR.START & mxcwtrad$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Max. radial cell wall thickness (MXCWTRAD) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #, ylim=c(1.5,10))
          par(xaxt="s")
        ...
        if (sum(mncwtrad$MNCWTRAD[mncwtrad$YEAR>=YR.START & mncwtrad$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNCWTRAD data
        {  
          ...
          plot(y=mncwtrad$MNCWTRAD[mncwtrad$YEAR>=YR.START & mncwtrad$YEAR<=YR.END],
               ...
               xaxt="n",  yaxt="n", xlab="",ylab="Min. radial cell wall thickness (MNCWTRAD) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
               cex.axis=1.3, cex.lab=2) #, ylim=c(1,6))
          par(xaxt="s")
        ...
        if (sum(mxmork$MXRTSR[mxmork$YEAR>=YR.START & mxmork$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXRTSR data
        {  
        ...
        if (sum(mnmork$MNRTSR[mnmork$YEAR>=YR.START & mnmork$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNRTSR data
        {  
        ...
        ### 4.6.1.85 Plot and save annual maximum relative anatomical density based on CWT (MXDCWT) ####
        if (sum(mxdcwt$MXDCWT[mxdcwt$YEAR>=YR.START & mxdcwt$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXDCWT data
        {  
        ...
        ### 4.6.1.86 Plot and save annual minimum relative anatomical density based on CWT (MNDCWT) ####
        if (sum(mndcwt$MNDCWT[mndcwt$YEAR>=YR.START & mndcwt$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNDCWT data
        {  
        ...
        ### 4.6.1.87 Plot and save annual maximum special anatomical density (MXDCWT2): CWTRAD/DRAD ####
        if (sum(mxdcwt2$MXDCWT2[mxdcwt2$YEAR>=YR.START & mxdcwt2$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXDCWT2 data
        {  
        ...
        ### 4.6.1.88 Plot and save annual minimum special anatomical density (MNDCWT2): CWTRAD/DRAD ####
        if (sum(mndcwt2$MNDCWT2[mndcwt2$YEAR>=YR.START & mndcwt2$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNDCWT2 data
        {  
        ...
        ### 4.6.1.89 Plot and save annual maximum relative anatomical density based on CWA (MXDCWA) ####
        if (sum(mxdcwa$MXDCWA[mxdcwa$YEAR>=YR.START & mxdcwa$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXDCWA data
        {  
        ...
        ### 4.6.1.90 Plot and save annual minimum relative anatomical density based on CWA (MNDCWA) ####
        if (sum(mndcwa$MNDCWA[mndcwa$YEAR>=YR.START & mndcwa$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNDCWA data
        {  
        ...
        ### 4.6.1.91 Plot and save annual maximum cell wall reinforcement index (MXTB2) ####
        if (sum(mxtb2$MXTB2[mxtb2$YEAR>=YR.START & mxtb2$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MXTB2 data
        {  
        ...
        ### 4.6.1.92 Plot and save annual minimum cell wall reinforcement index (MNTB2) ####
        if (sum(mntb2$MNTB2[mntb2$YEAR>=YR.START & mntb2$YEAR<=YR.END], na.rm=T) > 0)   #only plot if there is any non-NA MNTB2 data
        {  
        ...
        ### 4.6.1.93 Plot and save annual maximum mean hydraulic diameter (MXDH) ####
        ...
        plot(y=mxdh$MXDH[mxdh$YEAR>=YR.START & mxdh$YEAR<=YR.END],
             ...
             xaxt="n",  yaxt="n", xlab="",ylab="Max. mean hydraulic diameter (MXDH) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
             cex.axis=1.3, cex.lab=2) #, ylim=c(0.3,1))
        par(xaxt="s")
        ...
        ### 4.6.1.94 Plot and save annual minimum mean hydraulic diameter (MNDH) ####
        ...
        plot(y=mndh$MNDH[mndh$YEAR>=YR.START & mndh$YEAR<=YR.END],
             ...
             xaxt="n",  yaxt="n", xlab="",ylab="Min. mean hydraulic diameter (MNDH) (?m)", type="l", lwd=3, col=LINE.COL(length(RESO))[r],
             cex.axis=1.3, cex.lab=2) #, ylim=c(0.1,0.7))
        par(xaxt="s")
        ...
        ### 4.6.1.95 Plot and save annual maximum number of cells (MXN.BAND) ####
        ...
        ### 4.6.1.96 Plot and save annual minimum number of cells (MNN.BAND) ####
        ...
        ### 4.6.1.97 Plot and save annual maximum number of cells (MXN.TRUE) ####
        ...
        ### 4.6.1.98 Plot and save annual minimum number of cells (MNN.TRUE) ####
